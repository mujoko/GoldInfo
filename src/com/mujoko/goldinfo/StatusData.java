...
public class StatusData { // <1>
  ...
  public static final String C_BUY_RATE = "buy_rate";
  public static final String C_SELL_RATE = "sell_rate";
  static final String C_PROVIDER = "provider";
  ...
  private static final String[] DB_RATE_COLUMNS = { C_BUY_RATE };
    ...
    public void onCreate(SQLiteDatabase db) {
      Log.i(TAG, "Creating database: " + DATABASE);
		...
		String sql = "create table " + TABLE + " (" + C_ID
				+ " int primary key, " + C_CREATED_AT + " int, " + C_BUY_RATE
				+ " text, " + C_SELL_RATE
				+ " text, "  + C_PROVIDER + " text)"; // <6>
...
//          + C_CREATED_AT + " int, " + C_RATE + " text, " + C_TEXT + " text)");
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    	Log.d(TAG, "Drop Tables"+oldVersion);
      db.execSQL("drop table " + TABLE);
      this.onCreate(db);
    }
  }

  final DbHelper dbHelper; // <2>
  ...
  public String getStatusTextById(long id) {  // <11>
    SQLiteDatabase db = this.dbHelper.getReadableDatabase();
    try {
      Cursor cursor = db.query(TABLE, DB_RATE_COLUMNS, C_ID + "=" + id, null,
          null, null, null);
      try {
